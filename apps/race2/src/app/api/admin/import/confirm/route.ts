...
export async function POST(request: Request) {
  return guarded(async () => {
    ...
    if (!parsed.success) return jsonError("Neplatné parametry importu.", 400);
    const candidates = await loadRacerCandidates(parsed.data);
    const result = await importCandidates(parsed.data.raceId, candidates, parsed.data.dedupMode);
    return NextResponse.json({ created: result.created.length, skipped: result.skipped.length, racers: result.created });
  });
}
